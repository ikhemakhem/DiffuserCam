...
from diffcam.mirflickr import postprocess, postprocess2
...
def mirflickr_dataset(data, n_files, n_iter, single_psf, save):
    ...
    for fn in files:
        ...
        solver = Recon(diffuser_prep, psf_float, mode='ridge')
        ...
        plot_image(lensed)
        plt.show()
        print("lensed: ", type(lensed), lensed.shape)
        print("est: ", type(est), est.shape)
