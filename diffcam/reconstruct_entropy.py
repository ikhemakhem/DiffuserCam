...
def reconstruction(
    ...
    varlambda =  4
    entropy = varlambda*ShannonEntropy(dim = data.size)
    ...
    apgd = APGD(dim=data.size, F=F ,G=entropy, verbose=None) 
    # drs = DRS(dim=data.size, G = 0.5*entropy, H = 0.5*entropy, verbose = None)
    ####################
